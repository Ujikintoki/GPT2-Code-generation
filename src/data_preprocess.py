...
STRIDE = 256     # 调整为256（无重叠窗口，每个chunk独立）
...
def main():
    print("1. Loading CodeSearchNet (Python subset) 50% sample...")
    # 调整为抓取50%的训练数据（原1%改为50%）
    raw_datasets = load_dataset("code_search_net", "python", split="train[:50%]")
    ...
    print(f"Processing complete! {len(raw_datasets)} raw documents transformed into {len(lm_datasets)} chunks of length {MAX_LENGTH}.")
